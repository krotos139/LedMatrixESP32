...
#define LED_STRIP_REFRESH_PERIOD_MS     (50U) // TODO: add as parameter to led_strip_init
...
static rmt_item32_t rmt_items_data[32256];
...
static void led_strip_task(void *arg)
{
    ...
    rmt_item32_t *rmt_items = &rmt_items_data[0];//(rmt_item32_t*) malloc(sizeof(rmt_item32_t) * num_items_malloc);
	ESP_LOGI(TAG,"led_strip_task 1");
    if (!rmt_items) {
		ESP_LOGE(TAG,"Can not malloc (size:%d num_items_malloc:%d rmt_items:%p)", (sizeof(rmt_item32_t) * num_items_malloc), num_items_malloc, rmt_items);
        vTaskDelete(NULL);
    }
	...
	int sleep;
	// uint32_t clock_millis_p = (uint32_t) (clock() * 1000 / CLOCKS_PER_SEC)+1000;
	// int count = 0;
	// int sleep_min = 999;
    for(;;) {
		...
		clock_millis += LED_STRIP_REFRESH_PERIOD_MS;
		sleep = clock_millis-(clock() * 1000 / CLOCKS_PER_SEC);
		if (sleep>1) {
		...
		// if (sleep_min>sleep)
			// sleep_min = sleep;
		// if (clock_millis_p<=(clock() * 1000 / CLOCKS_PER_SEC)) {
			// clock_millis_p += 1000;
			// ESP_LOGI(TAG,"COUNT:%d SLEEP:%d SLEEPMIN:%d", count, sleep, sleep_min);
			// count = 0;
			// sleep_min = 999;
		// }
    }
